...
* **Actions:**
  ...
  * **Example Technical Story:** *As an integrations architect, I need the `MATMAS` IDoc payload to intercept via `BADI_MATERIAL_SAVE` before commit, so that the custom field `ZZ_ALT_BOM_ID` is mapped to the JSON payload heading for downstream FICO validation.*
